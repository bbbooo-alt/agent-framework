...
class MySimpleAgent(SimpleAgent):
    """
    重写的简单对话Agent
    展示如何基于框架基类构建自定义Agent，支持可选的工具调用功能
    """
    ...
    def _get_enhanced_system_prompt(self) -> str:
        ...
        # 获取工具描述
        tools_description = self.tool_registry.get_tools_description() if hasattr(self.tool_registry, 'get_tools_description') else "暂无可用工具"
    ...
    def _run_with_tools(
        ...
        **kwargs
        ...
        if current_iteration >= max_tool_iterations and not final_response:
            final_response = self.invoke(messages, **kwargs)
    ...
    def _parse_tool_calls(self, text: str) -> List[Dict[str, str]]:
        ...
        Returns:
            工具调用列表 [{'tool_name': str, 'parameters': str, 'original': str}]
        """
        pattern = r'\[TOOL_CALL:([^:]+):([^\]]+)\]'
    ...
    def _execute_tool_call(self, tool_name: str, parameters: str) -> str:
        ...
        try:
            ...
            if tool_name == 'calculator':
                ...
                tool = self.tool_registry.get_tool(tool_name) if hasattr(self.tool_registry, 'get_tool') else None
                if not tool:
    ...
    def stream_run(self, input_text: str, **kwargs):
        ...
        # 使用 llm_client 的 think 方法（已经是流式的）
    ...
    def add_tool(self, tool) -> None:
        ...
        """
        if not self.tool_registry:
            self.tool_registry = None  # ToolRegistry 将在后续实现
        ...
        if self.tool_registry and hasattr(self.tool_registry, 'register_tool'):
            ...
            print(f"⚠️ 工具注册表未配置，无法添加工具 '{tool.name}'")
